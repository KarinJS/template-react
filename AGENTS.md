...
下游开发者的模板都在 `ktr/template/` 文件夹里（App Router 式强约定：目录即路由，文件名固定），静态资源放 `ktr/public/`（dev server 当 publicDir 服务，构建时复制到产物 `assets/`；标记里和 CSS `url()` 里的 `/xxx` 引用恒等于 `<dir.assets>/xxx`，SSR 产出 HTML 时框架按 `html.assetsInlineLimit` 内联为 base64 或转为 `file://` 绝对路径，三态位置一致；CSS `url()` 走同一阈值，构建期 Vite 输出的 `/assets/*`（依赖包字体等）按产物目录解析）：
