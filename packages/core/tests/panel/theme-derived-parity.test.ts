...
describe('派生色公式对齐官方', () => {
  it('语义色 hover 统一混 10% 前景', () => {
    for (const key of ['accent', 'success', 'warning', 'danger']) {
      expect(readVar(lightBlock, `${key}-hover`)).toBe(
        `color-mix(in oklab, var(--${key}) 90%, var(--${key}-foreground) 10%)`
      )
    }
  })

  it('default 的 hover 与 soft 用中性色专属比例', () => {
    expect(readVar(lightBlock, 'default-hover')).toBe(
  ...
  it('soft-foreground 每种色的比例各不相同', () => {
    expect(readVar(lightBlock, 'accent-soft-foreground')).toBe(
      'color-mix(in oklab, var(--accent) 70%, var(--foreground) 30%)'
    )
    expect(readVar(lightBlock, 'success-soft-foreground')).toBe(
      'color-mix(in oklab, var(--success) 80%, var(--foreground) 60%)'
    )
    expect(readVar(lightBlock, 'warning-soft-foreground')).toBe(
      'color-mix(in oklab, var(--warning) 80%, var(--foreground) 70%)'
    )
    expect(readVar(lightBlock, 'danger-soft-foreground')).toBe(
  ...
  it('暗色下 soft-foreground 统一为 80/30', () => {
    for (const key of ['accent', 'success', 'warning', 'danger']) {
      expect(readVar(darkBlock, `${key}-soft-foreground`)).toBe(
        `color-mix(in oklab, var(--${key}) 80%, var(--foreground) 30%)`
      )
    }
  })
  ...
  it('表面与字段的派生公式与官方一致', () => {
    expect(readVar(lightBlock, 'surface-hover')).toBe(
