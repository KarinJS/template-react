...
1. **`.dark` 类名** —— ktr 在样式基座里用 `@custom-variant` 把 `dark:` 重定义为类选择器驱动（Tailwind v4 默认编译成 prefers-color-scheme 媒体查询，在截图浏览器的浅色偏好下永不命中）
...
export const card = <div className="bg-surface shadow-sm dark:shadow-none" />
