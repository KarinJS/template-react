...
路由写死成字面量时类型最安全。路由必须由用户输入（如 `#渲染模板 <路由>`）时，编译期绑不住 `data`，显式放宽后靠模板里的 `validate` 做运行时兜底；示例数据按"先 `data/default.json`、再 TS mock"的顺序读取。枚举全部模板用 `loadTemplateRegistry()`，用法见 [渲染器 API](/docs/api/renderer)。
