...
3. 生成 `template-registry.js` 和 `mock-registry.js` 用于生产环境
...
完整的构建配置和发布流程见 [构建发布](/docs/guide/building)。
...
<Callout type="error">
  ...
  **类型错误**：执行 `npx ktr sync` 重新生成类型定义 - **CSS 缺失**：确认构建配置挂了 `ktrBuildPlugin()` 且没有传 `{ css: false }` 更多帮助见
  [构建发布](/docs/guide/building) 和 [故障排查](/docs/troubleshooting)。
</Callout>
