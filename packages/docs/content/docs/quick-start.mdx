...
3. **类型安全**：`TemplateProps<HelloCardData>` 确保组件接收的数据结构类型安全
...
const renderTemplate = createTemplateRenderer(import.meta.url, {
  renderer: { outputDir: path.join(karinPathHtml, 'karin-plugin-example') }
})
...
export const renderImage = async <K extends keyof Registry & string>(
  ...
  const result = await render.render({
    name: `karin-plugin-example/${templatePath}`,
    file: htmlPath,
    selector: '#container',
    type: 'png',
    omitBackground: true,
