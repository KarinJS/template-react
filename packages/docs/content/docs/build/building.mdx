...
- **注册表**：渲染器自身跑在下游 bundle 里时（生产构建插件在产物 chunk 注入 `__KTR_BUNDLED__` 标记）直接用打包产物，不读 `.ktr` 源码也不加载 `karin.template.ts`；否则（ktr 作为 npm 包被开发态引用）`.ktr` 源文件优先，缺失时回退产物。产物按 `bundledDir` 选项 → `package.json` 的 `main` 目录 → 根目录扫描发现。
