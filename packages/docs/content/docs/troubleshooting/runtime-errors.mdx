...
真实渲染时报错，堆栈里组件的 React 和 `react-dom-server` 来自**不同的 node_modules 路径**：
...
1. 把 `@karinjs/template-react` 升级到包含此修复的版本；
...
如果自定义了额外样式路径，确保文件存在；也可以直接写依赖包里的样式（按 Node 解析，pnpm 把真实文件放在 `.pnpm` 下同样能命中）：
...
export default defineConfig({
  extraStylePaths: [
    ...
    '@lobehub/webfont-harmony-sans-sc/index-full.css' // 或依赖包里的样式
  ]
})
```

样式里 `url()` 引用的资源（字体、图片）超过 [`html.assetsInlineLimit`](/docs/api/config-cli#htmlassetsinlinelimit)（默认 4KB）时会转成 `file://` 绝对路径而不是 base64——字体"看起来没被内联"是预期行为，只要截图进程能读到那个文件就行；想连大字体一起内联就把阈值调大。
